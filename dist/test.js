...
var $jgrenat$elm_html_test_runner$Test$Runner$Html$Config = function (a) {
	return {$: 'Config', a: a};
};
var $jgrenat$elm_html_test_runner$Test$Runner$Html$Show = {$: 'Show'};
var $jgrenat$elm_html_test_runner$Test$Runner$Html$defaultConfig = function (initialSeed) {
	return $jgrenat$elm_html_test_runner$Test$Runner$Html$Config(
		{fuzzCount: 100, initialSeed: initialSeed, passedTestsDisplay: $jgrenat$elm_html_test_runner$Test$Runner$Html$Show});
};
...
var $jgrenat$elm_html_test_runner$Test$Runner$Html$showPassedTests = function (_v0) {
	var config = _v0.a;
	return $jgrenat$elm_html_test_runner$Test$Runner$Html$Config(
		_Utils_update(
			config,
...
var $jgrenat$elm_html_test_runner$Test$Runner$Html$viewInvalidReason = function (reason) {
	switch (reason.$) {
		case 'EmptyList':
			return $elm$html$Html$text('You should have at least one test in the list');
		case 'NonpositiveFuzzCount':
			return $elm$html$Html$text('The fuzz count must be positive');
		case 'InvalidFuzzer':
			return $elm$html$Html$text('The fuzzer used is invalid');
		case 'BadDescription':
			return $elm$html$Html$text('The description of your test is not valid');
		default:
			return $elm$html$Html$text('At least two tests have the same name, please change at least one');
	}
};
...
var $jgrenat$elm_html_test_runner$Test$Runner$Html$viewListDiffPart = F3(
	function (otherList, index, listPart) {
		var styles = A2(
			...
			A2(
				...
				A2(
					...
					function (value) {
						return _Utils_eq(value, listPart);
					},
					A2($elm$core$Array$get, index, otherList))));
		return A2(
			$elm$html$Html$li,
			styles,
			_List_fromArray(
				[
					$elm$html$Html$text(listPart)
				]));
	});
var $jgrenat$elm_html_test_runner$Test$Runner$Html$viewListDiff = F2(
	function (expected, actual) {
		var expectedArray = $elm$core$Array$fromList(expected);
		var actualArray = $elm$core$Array$fromList(actual);
		return A2(
			$elm$html$Html$div,
			_List_Nil,
			_List_fromArray(
				[
					$elm$html$Html$text('The lists don\'t match! 😱'),
					A2(
					$elm$html$Html$div,
					_List_fromArray(
						[
					...
					_List_fromArray(
						[
							...
							_List_fromArray(
								[
									A2($elm$html$Html$Attributes$style, 'margin-right', '5vw')
								]),
							_List_fromArray(
								[
									...
									A2(
										...
										$jgrenat$elm_html_test_runner$Test$Runner$Html$viewListDiffPart(actualArray),
										expected))
								])),
							A2(
							$elm$html$Html$div,
							...
							_List_fromArray(
								[
									...
									A2(
										$elm$core$List$indexedMap,
										$jgrenat$elm_html_test_runner$Test$Runner$Html$viewListDiffPart(expectedArray),
										actual))
								]))
						]))
				]));
	});
var $jgrenat$elm_html_test_runner$Test$Runner$Html$viewReason = function (reason) {
	switch (reason.$) {
		case 'Custom':
		...
		case 'Equality':
			...
			var actual = reason.b;
			return $elm$html$Html$text('Expected: ' + (expected + (' | Actual: ' + actual)));
		case 'Comparison':
			var expected = reason.a;
			var actual = reason.b;
			return $elm$html$Html$text('Expected: ' + (expected + (' | Actual: ' + actual)));
		case 'ListDiff':
			var expected = reason.a;
			var received = reason.b;
			return A2($jgrenat$elm_html_test_runner$Test$Runner$Html$viewListDiff, expected, received);
		case 'CollectionDiff':
			var details = reason.a;
			return $elm$html$Html$text('Expected: ' + (details.expected + (' | Actual: ' + details.actual)));
		case 'TODO':
			return $elm$html$Html$text('TODO');
		default:
			var invalidReason = reason.a;
			return $jgrenat$elm_html_test_runner$Test$Runner$Html$viewInvalidReason(invalidReason);
	}
};
var $jgrenat$elm_html_test_runner$Test$Runner$Html$viewExpectationResult = function (expectation) {
	return A2(
		$elm$core$Maybe$map,
		function (reason) {
			return A2(
				...
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								$elm$html$Html$text(reason.description)
							])),
						$jgrenat$elm_html_test_runner$Test$Runner$Html$viewReason(reason.reason)
					]));
		},
		$elm_explorations$test$Test$Runner$getFailureReason(expectation));
};
var $jgrenat$elm_html_test_runner$Test$Runner$Html$viewTestsResults = F2(
	function (passedTestsDisplay, runner) {
		var label = A3(
			$elm$core$List$foldl,
			$elm$core$Basics$append,
			'',
			A2($elm$core$List$intersperse, ' ', runner.labels));
		var getExpectationResults = function () {
			if (passedTestsDisplay.$ === 'Show') {
				return A2(
					$elm$core$Basics$composeR,
					$elm$core$List$map($jgrenat$elm_html_test_runner$Test$Runner$Html$viewExpectationResult),
					$elm$core$List$map(
						$elm$core$Maybe$withDefault(
							$elm$html$Html$text('Passed!'))));
			} else {
				return $elm$core$List$filterMap($jgrenat$elm_html_test_runner$Test$Runner$Html$viewExpectationResult);
			}
		}();
		return function (expectationsResults) {
			if (!$elm$core$List$length(expectationsResults)) {
				return A2(
					$elm$html$Html$div,
					_List_fromArray(
						[
							$elm$html$Html$Attributes$class('test-pass')
						]),
					_List_fromArray(
						[
							A2(
							...
							$elm$html$Html$text('✔️ All tests passed')
						]));
			} else {
							...
							return A2(
								...
								_List_fromArray(
									[
										...
										isFailure(result) ? 'test-fail' : 'test-pass')
									]),
								_List_fromArray(
									[
										A2(
...
var $jgrenat$elm_html_test_runner$Test$Runner$Html$viewResults = F2(
	function (_v0, testsToRun) {
		var config = _v0.a;
		var _v1 = A3($elm_explorations$test$Test$Runner$fromTest, config.fuzzCount, config.initialSeed, testsToRun);
		switch (_v1.$) {
			case 'Plain':
				var tests = _v1.a;
				return A2(
					...
					A2(
						$elm$core$List$map,
						$jgrenat$elm_html_test_runner$Test$Runner$Html$viewTestsResults(config.passedTestsDisplay),
						tests));
			case 'Only':
				var tests = _v1.a;
				return A2(
				...
				return A2(
					...
					_List_fromArray(
						[
							...
							_List_fromArray(
								[
									$elm$html$Html$text('Invalid tests, here is the reason:')
								])),
							A2(
							$elm$html$Html$pre,
							_List_Nil,
...
var $author$project$HtmlTests$main = A2(
	...
	_List_fromArray(
		[
			...
			_List_fromArray(
				[
					$elm$html$Html$text('My Test Suite')
				])),
			A2(
			$elm$html$Html$div,
			_List_Nil,
			_List_fromArray(
				[
					A2($jgrenat$elm_html_test_runner$Test$Runner$Html$viewResults, $author$project$HtmlTests$config, $author$project$HtmlTests$suite)
				]))
		]));
_Platform_export({'HtmlTests':{'init':_VirtualDom_init($author$project$HtmlTests$main)(0)(0)}});}(this));